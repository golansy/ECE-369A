...
module InstructionFetchUnit(Instruction, Reset, Clk, PCResult);
    ...
    PCAdder add0(.PCResult(PCResult), .PCAddResult(PCAddResult));
    ProgramCounter pc0(.Address(PCAddResult), .PCResult(PCResult), .Reset(Reset), .Clk(Clk));
    InstructionMemory imem0(.Address(PCResult), .Instruction(Instruction));
endmodule
